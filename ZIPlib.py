...
class ZIP:
	def __init__(self, row):
		...
		self.decommissioned = True if ZIP.__read_remove_value(row,'decommissioned','0') == '1' else  False
		self.primary_city = ZIP.__read_remove_value(row,'primary_city')
		self.acceptable_cities = ZIP.__read_remove_value(row,'acceptable_cities')[1:-2].split(',')
		self.unacceptable_cities = ZIP.__read_remove_value(row,'unacceptable_cities')[1:-2].split(',')
		...
		self.irs_estimated_population = int(ZIP.__read_remove_value(row,'irs_estimated_population','0'))
		self.other_vals = row
...
class __ZIPlib:
	...
	def find_ZIP(self, ZIP_code):
		for zip in self.prefixes[ZIP_code[:3]]['ZIPs']:
			if zip.ZIP_code == ZIP_code: return zip
	...
	def __should_be_culled(self,zip, diallowed_prefixes):
		if zip.decommissioned: return True
		if zip.state_abbr not in STATE_ABBR_MAP: return True
		if self.__zip_any_prefix(diallowed_prefixes, zip): return True
		if self.__zip_any_prefix(IRS_PREFIXES + MILIARTY_PREFIXES, zip): return True
	...
	def __group_prefixes(self, num_digits):
		for zip in self.ZIPs:
			if zip.state_abbr == 'HI' or zip.state_abbr == 'AK': continue # my map doesn't show Hawaii or Alaska
			if zip.ZIP_code[:3] == '569': continue # parcel return in DC, not upper midwest
			if zip.ZIP_code == '88888': continue # North Pole, not Rockies
			if float(zip.latitude) == 0: continue
			if float(zip.longitude) == 0: continue
	...
	def __compute_convex_hull(self, prefix):
		self.prefixes[prefix]['ZIPs'] = sorted(self.prefixes[prefix]['ZIPs'], key=lambda zip: zip.latitude)
	...
	def __compute_hull(self,sorted_zips, cmp):
		...
		for zip in sorted_zips:
			...
			if len(hull) == 0 or cmp(zip, hull[peak_i]) or (zip.longitude == hull[peak_i].longitude and zip.latitude != hull[peak_i].latitude):
				peak_i = len(hull)
